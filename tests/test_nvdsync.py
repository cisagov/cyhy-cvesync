...
async def test_fetch_cve_data_invalid_url_scheme(mock_urlopen):
    ...
    with pytest.raises(ValueError, match="Invalid URL scheme in CVE JSON URL: ftp"):
        await fetch_cve_data(cve_json_url, gzipped=False)


@patch("urllib.request.urlopen")
async def test_fetch_cve_data_json_decode_error(mock_urlopen):
    ...
    with pytest.raises(json.JSONDecodeError):
        await fetch_cve_data("https://example.com/nvd.json", gzipped=False)
...
async def test_fetch_real_cve_data():
    ...
    cve_json = await fetch_cve_data(cve_url, gzipped=True)
